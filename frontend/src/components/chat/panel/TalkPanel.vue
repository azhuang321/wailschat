...
export default {
    ...
    methods: {
        ...
        onKeyboardEvent(text) {
            ...
            // 判断当前对话是否属于私聊信息
            if (this.params.talk_type == 2 || !this.isOnline) {
                return;
            }
        ...
        handleMultiMode(value) {
            ...
            if (value == 'forward' || value == 'merge_forward') {
                ...
                // 批量删除
                const ids = this.multiSelect.items;
                ServeRemoveRecords({
                    talk_type: this.params.talk_type,
                    receiver_id: this.params.receiver_id,
                    record_id: ids.join(',')
        ...
        compareTime(index, datetime) {
            ...
            const nextDate = this.records[index + 1].created_at.replace(/-/g, '/');
        ...
        triggerMultiSelect(records_id) {
            const i = this.multiSelect.items.indexOf(records_id);
            if (i >= 0) {
        ...
        onCopy(idx, item, event) {
            ...
            if (item.user_id == this.uid) {
                const time = new Date().getTime() - Date.parse(item.created_at.replace(/-/g, '/'));
